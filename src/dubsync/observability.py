...
def name_spelling_inconsistency_flags(
    ...
    source_tokens = [token for cue in source_cues for token in _case_preserving_tokens(cue.plain_text)]
    ...
    for output_key, occurrences in sorted(output_occurrences.items()):
        ...
        flags.append(
            QCFlag(
                kind="name_spelling_inconsistency",
                ...
                message=(
                    ...
                    f"'{source_spelling}' appears {source_counts[source_key]} times; review as a possible name drift."
                ),
                severity="warning",
