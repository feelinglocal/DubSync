...
Historical context: no plan file was supplied. The retired pilot journey was derived from a past request to compare Gemini 3.5 Transcribe without replacing the default ASR provider.

## Historical User Journeys

- During the retired pilot, the testing toggle was visible only when the server enabled it and a Gemini key was configured.
- During the retired pilot, a sync or audio-to-SRT job could opt into `gemini-3.5-transcribe`; unchecked jobs kept the default provider.
- During the retired pilot, queued Gemini jobs failed closed if the testing flag was later disabled.

## Archived Task Report

The table below records what the retired pilot once tested. These are not current product guarantees; current code rejects Gemini 3.5 Transcribe ASR everywhere.

| # | What the retired pilot tested | Test file or command | Test type | Historical result | Evidence |
|---|--------------------|----------------------|-----------|--------|----------|
...
## Archived Security Notes

The following controls describe the retired pilot implementation and are retained only for audit history. They no longer describe an available provider path.
...
## Archived Coverage And Known Gaps
