...
const privacySections: Section[] = [
  ...
  { title: '4. Service providers', paragraphs: ['DubSync runs primarily on Render infrastructure. Speech recognition sends audio to ElevenLabs Scribe v2 by default, or to Microsoft MAI-Transcribe 2 through OpenRouter when you select MAI. Processing can also send content to Google Gemini for audio-aware adjudication and bounded punctuation of dialogue, and OpenAI for speaker mapping. Other providers are used only when configured for a specific job. Their processing is governed by their own terms and data practices.', 'DubSync does not promise zero retention by a provider unless that setting is confirmed for the relevant account and job.'] },
